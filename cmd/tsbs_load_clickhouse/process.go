...
func (p *processor) processCSI(tableName string, rows []*insertData) (uint64, float64) {
	if !useTSModel {
		...
		return p.processTSModel(tableName, rows)
	}
}
...
func (p *processor) processColumnModel(tableName string, rows []*insertData) (uint64, float64) {
	...
	commonTagsLen := len(tagCols["cpu_tags_metrics_logical_distributed"])
	colLen := len(tableCols["cpu_tags_metrics_logical_distributed"]) + 1 // +1 to add time col
	ret := uint64(0)
	...
	/*
		...
		p.csi.mutex.Lock()
		hostnameToTags := insertTags(p.db, len(p.csi.m), dataRows, true)
	...
	*/
	...
	cols = append(cols, tagCols["cpu_tags_metrics_logical_distributed"]...)
	cols = append(cols, "time")
	cols = append(cols, metricCols["cpu_tags_metrics_logical_distributed"]...)

	tx := p.db.MustBegin()
	// INSERT statement template
	sql := fmt.Sprintf("INSERT INTO %s.cpu_tags_metrics_logical_distributed (%s) VALUES (%s)",
		loader.DBName,
		strings.Join(cols, ","),
...
func (p *processor) processTSModel(tableName string, rows []*insertData) (uint64, float64) {
	tagRows := make([][]string, 0, len(rows))
	dataRows := make([][]interface{}, 0, len(rows))
	commonTagsLen := len(tagCols["cpu_tags_metrics"])
	colLen := commonTagsLen + 3 // +3 to add time col and metric_name col and value col
	metricNames := metricCols["cpu_tags_metrics"]
	ret := uint64(0)
	...
	for _, data := range rows {
		...
		for i, v := range metrics[1:] {
			...
			for index, value := range tags {
				variadicArgs[index] = convertBasedOnType(tagColumnTypes[index], value)
			}
			variadicArgs = append(variadicArgs, timeUTC)
			...
			variadicArgs = append(variadicArgs, metricNames[i])
			variadicArgs = append(variadicArgs, f64)
			dataRows = append(dataRows, variadicArgs)
		}
	}
